...
LEARNING_RATE      = 3e-5
NUM_EPOCHS         = 2
OUTPUT_DIR         = "./finetuned_models/roberta-base-squad2-nq"
# USE_BF16           = True                            # 4080 Super supports bfloat16
USE_GRAD_CHKPT     = True                            # memory saver on long seqs
base_model_dir     = model_path() / "deepset-roberta-base-squad2"
...
K = 1  # try 1, else 2
def is_lastK(n): 
    return any(re.search(fr"encoder\.layer\.{i}\.", n) for i in range(12-K, 12))
...
for n,p in model.named_parameters():
    ...
    elif is_lastK(n) and ("attention" in n or "LayerNorm" in n):
        p.requires_grad = True; lastk_params.append(p)
    else:
        p.requires_grad = False

optimizer = AdamW([
    {"params": head_params, "lr": 3e-5},
    {"params": lastk_params, "lr": 5e-6},
], weight_decay=0.01)
...
def postprocess_logits_to_text(examples, features, predictions, n_best=20, max_answer_len=30):
